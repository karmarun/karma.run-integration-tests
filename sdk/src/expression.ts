...
function dataForNumberExpression(expr: t.NumberExpression, type: NumberType): t.Expression {
  if (typeof expr === 'number') {
    switch (type) {
      default:
      case NumberType.Int8: return expression.int8(expr)
      case NumberType.Int16: return expression.int16(expr)
      case NumberType.Int32: return expression.int32(expr)
      case NumberType.Int64: return expression.int64(expr)
      case NumberType.UInt8: return expression.uint8(expr)
      case NumberType.UInt16: return expression.uint16(expr)
      case NumberType.UInt32: return expression.uint32(expr)
      case NumberType.UInt64: return expression.uint64(expr)
      case NumberType.Float: return expression.float(expr)
    }
  } else {
    return expr
  }
...
function generateNumericTupleFunc<T extends {}>(name: keyof T, type: NumberType) {
  return (valueA: t.NumberExpression, valueB: t.NumberExpression): T => {
    return {[name]: [
      dataForNumberExpression(valueA, type),
      dataForNumberExpression(valueB, type)
    ]} as T
  }
}
...
export const data = {
  ...
  ref(modelID: string, recordID: string): t.RefFn {
...
export const expression = {
  ...
  joinString(separator: t.Expression, strings: t.Expression): t.JoinStringsFn {
  ...
  key(name: t.Expression, value: t.Expression): t.KeyFn {
  ...
  memSort(value: t.Expression, expression: t.Expression): t.MemSortFn {
  ...
  reduceList(value: t.Expression, initial: t.Expression, reducer: t.FuncExpression): t.ReduceListFn {
  ...
  with(retrn: t.Expression, value: t.Expression): t.WithFn {
